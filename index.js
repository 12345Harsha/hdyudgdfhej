  ...
  vapiSocket.on('message', (msg) => {
    if (Buffer.isBuffer(msg)) {
      ...
      try {
        ...
        console.log('📩 Vapi sent non-binary message');
      }
    }
  });

  // Receive audio from TeleCMI → send to Vapi
  ws.on('message', (msg) => {
    if (vapiSocket?.readyState === WebSocket.OPEN) {
      vapiSocket.send(msg);
      console.log('📤 TeleCMI → Vapi (raw audio)');
    } else {
